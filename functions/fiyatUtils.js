...
const ERKEN_KAYIT_BITIS = new Date('2026-08-31T23:59:59+03:00');

function kdvHesapla(net, kdvOrani = VARSAYILAN_KDV) {
  return Math.round(net * kdvOrani) / 100;
}

function erkenKayitAktifMi(simdi = new Date()) {
...
function kodDogrulaIc(kodDoc, email, telefon) {
  ...
  if (kodDoc.usedCount >= kodDoc.maxUses) return { gecerli: false, sebep: 'Kullanım limiti dolmuş.' };
  if (kodDoc.validUntil) {
    const bitis = kodDoc.validUntil.toDate ? kodDoc.validUntil.toDate() : new Date(kodDoc.validUntil);
    if (bitis < new Date()) return { gecerli: false, sebep: 'Kodun süresi dolmuş.' };
  }
  const emailEslesti =
    kodDoc.allowedEmail && email &&
    kodDoc.allowedEmail.trim().toLowerCase() === email.trim().toLowerCase();
  const telefonEslesti =
    kodDoc.allowedPhone && telefon &&
...
function temelFiyat(simdi) {
  if (erkenKayitAktifMi(simdi)) {
    ...
    return { kaynak: 'erken_kayit', net: ERKEN_KAYIT_NET, kdvOrani: VARSAYILAN_KDV, kdvTutari, brut: ERKEN_KAYIT_NET + kdvTutari };
  }
  const kdvTutari = kdvHesapla(STANDART_NET);
  return { kaynak: 'standart', net: STANDART_NET, kdvOrani: VARSAYILAN_KDV, kdvTutari, brut: STANDART_NET + kdvTutari };
}

function fiyatHesapla({ kodDoc = null, email = '', telefon = '', simdi = new Date() } = {}) {
  if (kodDoc) {
    ...
    if (gecerli) {
      ...
      return { kaynak: 'ozel_kod', net, kdvOrani, kdvTutari, brut: net + kdvTutari, kodGecerli: true, kodSebep: null };
    }
...
module.exports = { fiyatHesapla, kdvHesapla, erkenKayitAktifMi, STANDART_NET, ERKEN_KAYIT_NET, VARSAYILAN_KDV };
